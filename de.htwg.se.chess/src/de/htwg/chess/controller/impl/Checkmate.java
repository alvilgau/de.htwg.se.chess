...
public class Checkmate {
	...
	public static enum CheckMate {save, check, mate;
	...
	public boolean isMate() {
		return checkmateWhite == CheckMate.mate || checkmateBlack == CheckMate.mate;
	}
	 ...
	 * @param teamWhite - list of all figures of the white team
	 * @param teamBlack - list of all figures of the black team
	 ...
	 * @param fields - the current playground
	 */
	private void updateTeamWhite(List<IFigure> teamWhite, List<IFigure> teamBlack, IField fields[][]) {
		int xPos = teamWhite.get(0).getxPos();
		int yPos = teamWhite.get(0).getyPos();
		boolean updated = false;
		
		for(IFigure fig : teamBlack) {
			if(fig.possibleMove(xPos, yPos, fields)) {
				nextStateWhite();
				updated = true;
				break;
			}
	 ...
	 * @param fields - the current playground
	 */
	private void updateTeamBlack(List<IFigure> teamWhite, List<IFigure> teamBlack, IField fields[][]) {
	...
	public String getStatusMessage() {	
		switch (checkmateWhite) {
			case save: statusMessageWhite = "";
				break;	
			case check: statusMessageWhite = "White King is in a check! ";
				break;
			default: statusMessageWhite = "White King is mate. Team Black has won. - Game Over! ";
				break;
		}
		
		switch (checkmateBlack) {
			case save: statusMessageBlack = "";
				break;	
			case check: statusMessageBlack = "Black King is in a check!";
				break;
			default: statusMessageBlack = "Black King is mate. Team White has won. - Game Over!";
				break;
		}
