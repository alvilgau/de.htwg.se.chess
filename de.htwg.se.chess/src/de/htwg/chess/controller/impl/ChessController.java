...
public class ChessController extends Observable implements IChessController {
	...
	private void initTeamOne() {
		...
		for(int i = 0; i <= SEVEN; i++) {
			...
			fields[i][ONE] = new Field(true, figuresTeamWhite.get(FIELD_SIZE+i));
		}
	}
	...
	private void initTeamTwo() {
		...
		for(int i = 0; i <= SEVEN; i++) {
			...
			fields[i][SIX] = new Field(true, figuresTeamBlack.get(FIELD_SIZE+i));
		}
	}
	 ...
	 * @param exchange - new exchange status
	 */
	 ...
	 * @param gameover - new game over status
	 */
	...
	public void select(int x, int y) {		
		if(fields[x][y].getFigur() != null && turn == fields[x][y].getFigur().getTeamNumber()) {
	...
	public void exchangeKnight() {
		...
		if(team == Team.black) {
			...
			fields[xPos][yPos].setFigur(figuresTeamBlack.get(figuresTeamBlack.size()-1));
		}
		else {
			figuresTeamWhite.remove(moveFigure);
			figuresTeamWhite.add(new Knight(xPos, yPos, team));
			fields[xPos][yPos].setFigur(figuresTeamWhite.get(figuresTeamWhite.size()-1));
		}
