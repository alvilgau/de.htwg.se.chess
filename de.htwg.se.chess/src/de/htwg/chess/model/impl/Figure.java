...
public abstract class Figure implements IFigure {
	 ...
	 * @param white - URI to the white image
	 * @param black - URI to the black image
	 */
	protected void loadImages(String white, String black) {
		try {
			whiteFigure = ImageIO.read(new File(white));
			blackFigure = ImageIO.read(new File(black));
		} catch (IOException e) {
	...
	protected boolean possibleKill(int x, int y, IField fields[][]) {
		if(fields[x][y].isSet() && getTeamNumber() == fields[x][y].getFigur().getTeamNumber()) {
				return false;
		}
